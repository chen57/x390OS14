...
- 目标机器：Lenovo X390 (i3-8th 系列假设)；有线网卡：Intel（你已选择 Intel）。
- OpenCore 版本：latest（请在本地替换为你实际使用的版本并下载对应驱动/工具/ACPI）。

主要文件
- EFI/OC/config.plist — 注释版模板（含占位 PlatformInfo，请本地替换序列号/MLB/ROM/SmUUID）
- download-kexts.ps1 — PowerShell 脚本：从官方 GitHub releases 拉取常用 kext 并放到 EFI/OC/Kexts（需在 Windows/PowerShell 或有 PowerShell Core 的环境运行）
- EFI/OC/ACPI/ — 占位 SSDT 文件
- EFI/BOOT/BOOTx64.efi — 占位说明（请用你编译/下载的实际 OpenCore BOOTx64.efi 替换）
...
- 在推送 PR 前请自行检查并替换所有占位值与对应 OpenCore 驱动版本。
...
2) 将本仓 scaffold 写入后：
   git add .
   git commit -m "Add OpenCore EFI scaffold for X390 (i3-8th, Intel 9560)"
